# Requirement: Account Document Tracking

## Business need

The business needs to keep basic information about documents related to a client directly in Salesforce, attached to the client's Account record (e.g. ID cards, contracts, invoices, certificates).
...
3. **Document Type values** (default proposal): ID Card, Passport, Contract, Invoice, Certificate, Other.
4. **Document Status values** (default proposal): Draft, Valid, Expired, Revoked (default: Draft).
5. **Object API name: `Account_Document__c`** — chosen to avoid colliding with the existing `Client_Document__c` object already in this org/repo (added separately by another contributor covering the same business need). If both objects are not meant to coexist long-term, this should be reconciled with the business/team afterward.
